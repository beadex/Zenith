...
void ZenithRenderEngine::OnInit()
{
    ...
    m_model = std::make_unique<Model>(device, "Assets/Models/cnek.obj");
...
void ZenithRenderEngine::OnRender(const Timer& timer)
{
	...
	// 3. Calculate Model-View-Projection (MVP) matrix and update the constant buffer
    ...
    m_cbData.mvp = XMMatrixTranspose(model * view * projection);

	// 4. Update constant buffer with the latest transformation matrices (Model-View-Projection)
    commandList->SetGraphicsRootConstantBufferView(0, m_constantBuffer->GetGPUVirtualAddress());

	// 5. Set up scissor rect and viewport
    commandList->RSSetViewports(1, &m_viewport);
    commandList->RSSetScissorRects(1, &m_scissorRect);

	// 6. Record commands to clear the render target and set it for drawing. You can also set the viewport and scissor rect here if needed.
	...
	// 8. Record commands to draw geometry here (Set pipeline state, set root signature, set vertex/index buffers, draw calls, etc.)
